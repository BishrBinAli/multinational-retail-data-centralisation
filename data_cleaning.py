...
class DataCleaning:
    def clean_user_data(self, user_df):
        ...
        # Changing join_date and date_of_birth values to null where join_date is before date_of_birth
        cleaned_user_df.loc[cleaned_user_df['date_of_birth'] > cleaned_user_df['join_date'], [
            'join_date', 'date_of_birth']] = np.nan

        # Converting country and country code columns to category type
        cleaned_user_df['country'] = cleaned_user_df['country'].astype(
            'category')
        cleaned_user_df['country_code'] = cleaned_user_df['country_code'].astype(
            'category')
        ...
        cleaned_user_df['country_code'] = cleaned_user_df['country'].map(
            country_code_dict)
        ...
        cleaned_user_df[['phone_number', 'phone_extension']] = cleaned_user_df.apply(
            format_phone_num, axis=1, result_type='expand')
    ...
    def clean_card_data(self, card_df):
        ...
        card_df = card_df[~card_df['card_number'].str.contains(
            '[a-zA-z]', na=False, regex=True)]

        # Removing '?' from card_numbers
        card_df['card_number'] = card_df['card_number'].astype(
            'str').str.replace("?", "")

        # Convert date column to datetype
        card_df['date_payment_confirmed'] = pd.to_datetime(
            card_df['date_payment_confirmed'], infer_datetime_format=True, errors='coerce')
    ...
    def clean_store_data(self, store_df):
        ...
        store_df.loc[store_df['continent'].str.contains(
            'Europe', na=False), 'continent'] = 'Europe'
        store_df.loc[store_df['continent'].str.contains(
            'America', na=False), 'continent'] = 'America'

        # Remove alphabets from staff_numbers
        store_df['staff_numbers'] = store_df['staff_numbers'].str.replace(
            '\D', "", regex=True)
        ...
        store_df['opening_date'] = pd.to_datetime(
            store_df['opening_date'], infer_datetime_format=True, errors='coerce')
    ...
    def convert_product_weights(self, products_df):
        ...
        # Removing full stops(.)
        products_df['weight'] = products_df['weight'].str.replace(
            r'(?<=\D)\.|(?=(\D))|\.$', "", regex=True)
        ...
        products_df['unit'] = products_df['weight'].str.extract(
            r'([A-Za-z]+$)')
        products_df['value'] = products_df['weight'].str.replace(
            r'([A-Za-z]+$)', "", regex=True)
    ...
    def clean_products_data(self, products_df):
        ...
        products_df = products_df[products_df['uuid'].str.fullmatch(
        ...
        products_df['date_added'] = pd.to_datetime(
            products_df['date_added'], infer_datetime_format=True, errors='coerce')
    ...
    def clean_date_events_data(self, date_events_df):
        ...
        date_events_df = date_events_df[date_events_df['date_uuid'].str.fullmatch(
            r'^[0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12}$')]

        # Converting timestamp column to datetime.time type
        date_events_df['timestamp'] = pd.to_datetime(
            date_events_df['timestamp']).dt.time
        ...
        date_events_df['time_period'] = date_events_df['time_period'].astype(
            'category')
